...
class BlocCadastroPrestadorDeServico {
  RespostaProcessamento gravaCadastroPrestadorDeServico(
      ViewModelCadastroPrestadorDeServico viewModel) {
    RespostaProcessamento respostaProcessamento =
        _validaInformacoesDigitadasCorretamente(viewModel);
    if (respostaProcessamento.processamentoOK == false)
      return respostaProcessamento;

    BusinessModelPrestadorDeServicos businessModel =
  ...
  RespostaProcessamento _validaInformacoesDigitadasCorretamente(
      ViewModelCadastroPrestadorDeServico viewModel) {
    ...
    RespostaProcessamento respostaValidaTelefone =
        _validaNumeroDeTelefone(viewModel.telefone.text);
    if (respostaValidaTelefone.processamentoOK == false)
      return respostaValidaTelefone;
  ...
  RespostaProcessamento _validaNome(String nome) {
    if (nome.length >= 4) {
      ...
      return RespostaProcessamento.erro(
          "Nome inválido! O nome precisa ter 4 caracteres!");
    }
  }
  ...
  RespostaProcessamento _validaNumeroDeTelefone(String telefone) {
    ...
    NumeroDeTelefone numeroDeTelefone =
        NumeroDeTelefone.parse(telefone, listaDosDDD);
